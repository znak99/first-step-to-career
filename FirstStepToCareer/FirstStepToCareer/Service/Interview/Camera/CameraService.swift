...
public final class CameraService: NSObject, CameraServicing, @unchecked Sendable {
    ...
    public private(set) var frames: AsyncStream<CVPixelBuffer>

    // 입력/디바이스를 보관했다가 shutdown 시 해제
    private var currentInput: AVCaptureDeviceInput?

    public override init() {
        var localContinuation: AsyncStream<CVPixelBuffer>.Continuation?
        self.frames = AsyncStream<CVPixelBuffer> { cont in
...
extension CameraService: AVCaptureVideoDataOutputSampleBufferDelegate {
    public func captureOutput(_ output: AVCaptureOutput,
        ...
        // Vision/ARKit에서 바로 쓰기 좋게 픽셀 버퍼로 전달
